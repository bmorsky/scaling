...
Y₀ = 1
N = 1
γ = 0.5 # default budget γ=1
# β = 1.2 # superlinear scaling
β = 0.8 # sublinear scaling

# Calculate utility
function calc_utility(u)
    utility = zeros(L^2)
    for i=1:L
        for j=1:L
            utility[L*(i-1)+j] = log(1.0 + γ*u[L*(i-1)+j]^(1-β)/(Y₀*N^β))
        end
    end
    return utility
end
...
prob = ODEProblem(replicator!,u0,tspan)
sol = solve(prob,abstol=1e-20,reltol=1e-20,saveat=1)

# Format data
population = vcat(hcat(u0, zeros(L^2)),zeros(Int(T)*(L^2),2))
social_welfare = vcat(hcat(calc_utility(u0), zeros(L^2)),zeros(Int(T)*(L^2),2))
global count = L^2
for j = 2:T+1
    for i = 1:L^2
        global count = count + 1
        social_welfare[count,:] = [calc_utility(sol[:,j])[i], j]
...
# p1 = heatmap(heat, title = L"Population at time $t=100$", xaxis=false, yaxis=false, colorbar_title=L"Population, $p_i$", c = cgrad([RGB(1,1,1),RGB(1,113/255,206/255)]), clim=(0.0,0.05001),aspect_ratio=:equal)
...
# plot!(collect(0:T),Rawlsian_social_welfare, label = L"Rawlsian, $W_R$", linewidth=3, color=RGB(185/255,103/255,1),legend=:topright,ylims=(1.8,2.6))
...
# Sublinear plotting
p1 = heatmap(heat, title = L"Population at time $t=300$", xaxis=false, yaxis=false, colorbar_title=L"Population, $p_i$", c = cgrad([RGB(1,1,1),RGB(1,113/255,206/255)]), clim=(0.0,0.5001),aspect_ratio=:equal)
p2 = plot(collect(0:T), lineSW, title= L"Utility $U_i$ vs time $t$", label=false, xlabel=L"Time, $t$", ylabel=L"Utility, $U_i$", color = :black, alpha=0.3)
plot!(collect(0:T),city_social_welfare, label = L"Average city utility, $W_C$", linewidth=3, color=RGB(1,113/255,206/255))
plot!(collect(0:T),utilitarian_social_welfare, label = L"Utilitarian, $W_U$", linewidth=3, color=RGB(1/255,205/255,254/255))
plot!(collect(0:T),Nash_social_welfare, label = L"Nash, $W_N$", linewidth=3, color=RGB(5/255,1,161/255), linestyle=:dash)
plot!(collect(0:T),Rawlsian_social_welfare, label = L"Rawlsian, $W_R$", linewidth=3, color=RGB(185/255,103/255,1),legend=:topleft,ylims=(-0.05,1.05))
p3 = plot(collect(0:T), linePop, title= L"Population $p_i$ vs time $t$", label=false, xlabel=L"Time, $t$", ylabel=L"Population, $p_i$", color = :black, alpha=0.3)
plot!(collect(0:T),avg_population, label = L"Average population, $\bar{p}$", linewidth=3, color=RGB(1,113/255,206/255),legend=:topleft,ylims=(-0.05,1.05))

plot(p1, p2, p3, layout=grid(1, 3), size=(1000,330))
# savefig("superlinear_one_resource.pdf")
# savefig("sublinear_one_resource.pdf")
# savefig("superlinear_one_resource_$γ.pdf")
savefig("sublinear_one_resource_$γ.pdf")