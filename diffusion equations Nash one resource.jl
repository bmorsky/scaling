...
@variable(model, 1 .>= X[1:L^2] .>= ϵ)

@constraint(model, sum(X[i] for i in 1:L^2) == Budget)

# Determine allocations that maximize Nash social welfare
function Nash_allocation(u)
    set_start_value.(X[1:L^2], u)

    # Objective: maximize Nash social welfare
    @NLobjective(model, Max, prod(log(1.0 + X[i] / (Y₀ * (u[i] * N)^β))^(u[i]) for i in 1:L^2))
...
function calc_utility(u)
    ...
    for i = 1:L
        for j = 1:L
            utility[L*(i-1)+j] += log(1.0 + allocation[L*(i-1)+j] / (Y₀ * (u[L*(i-1)+j] * N)^β))
        end
    end
    return utility
end
...
sol = solve(prob, Tsit5(), abstol=1e-15, reltol=1e-15, saveat=1) #abstol=1e-20,reltol=1e-20
...
# plot!(collect(0:T),Rawlsian_social_welfare, label = L"Rawlsian, $W_R$", linewidth=3, color=RGB(185/255,103/255,1),legend=:topright,ylims=(0.55,1.0))
...
# Sublinear plotting
p1 = heatmap(heat, title=L"Population at time $t=100$", xaxis=false, yaxis=false, colorbar_title=L"Population, $p_i$", c=cgrad([RGB(1, 1, 1), RGB(1, 113 / 255, 206 / 255)]), clim=(0.0, 0.02501), aspect_ratio=:equal)
p2 = plot(collect(0:T), lineSW, title=L"Utility $U_i$ vs time $t$", label=false, xlabel=L"Time, $t$", ylabel=L"Utility, $U_i$", color=:black, alpha=0.3)
plot!(collect(0:T), city_social_welfare, label=L"Average city utility, $W_C$", linewidth=3, color=RGB(1, 113 / 255, 206 / 255))
plot!(collect(0:T), utilitarian_social_welfare, label=L"Utilitarian, $W_U$", linewidth=3, color=RGB(1 / 255, 205 / 255, 254 / 255))
plot!(collect(0:T), Nash_social_welfare, label=L"Nash, $W_N$", linewidth=3, color=RGB(5 / 255, 1, 161 / 255), linestyle=:dash)
